    ...
    with Progress(transient=True) as progress:
        for fs, mxs in masses.items():
            ...
            constraints[fs] = singlechannel.compute(
                mxs, fs, decay=True, progress=progress, gecco=False, cmb=False
            )
            for sigma in sigmas:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        prog="generate_single_channel_dec",
        description="Compute constraints on the single-channel decay model.",
        parents=[base_parser],
    )
